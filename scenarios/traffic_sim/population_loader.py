...
import sumolib
from agent import Activity, ActivityType, Agent, TransportMode
...
class PopulationLoader:
    ...
    def _create_schedule_from_trips(self, trips: list[dict], home_edge: str) -> list[Activity]:
        ...
        if trips:
            first_trip = trips[0]
            first_depart = (
                int(first_trip["depart_time_hours"]) * 3600
                + int(first_trip["depart_time_minutes"]) * 60
            )
            ...
            for i, trip in enumerate(trips):
                ...
                transport_mode = self._validate_and_adjust_mode(
                    origin_edge, dest_edge, transport_mode
                )

                # Calculate trip start time
                depart_time = (
                    int(trip["depart_time_hours"]) * 3600 + int(trip["depart_time_minutes"]) * 60
                )

                # Duration = time until next trip (or end of day)
                if i < len(trips) - 1:
                    next_depart = (
                        int(trips[i + 1]["depart_time_hours"]) * 3600
                        + int(trips[i + 1]["depart_time_minutes"]) * 60
                    )
    ...
    def _validate_and_adjust_mode(
        self, from_edge: str, to_edge: str, mode: TransportMode
    ) -> TransportMode:
    ...
    def _can_route_between(self, from_edge: str, to_edge: str) -> bool:
        ...
        try:
            ...
            can_route = (
                from_edge_obj is not None
                and to_edge_obj is not None
                and len(from_edge_obj.getOutgoing()) > 0
            )
    ...
    def _select_transport_mode(self, age: int, distance_km: float = None) -> TransportMode:
        ...
        if age < 16:
            ...
            return random.choices(
                [TransportMode.WALK, TransportMode.CAR, TransportMode.BUS], weights=[0.4, 0.4, 0.2]
            )[0]
        elif age < 18:
            ...
            return random.choices(
                [TransportMode.CAR, TransportMode.BUS, TransportMode.WALK], weights=[0.5, 0.3, 0.2]
            )[0]
    ...
    def _create_work_schedule(self, home_edge: str, work_edge: str, age: int) -> list[Activity]:
        ...
        return [
            ...
            Activity(
                ActivityType.HOME, home_edge, int(return_time), 24 * 3600 - int(return_time), mode
            ),
        ]
    ...
    def _create_school_schedule(self, home_edge: str, school_edge: str, age: int) -> list[Activity]:
        ...
        return [
            Activity(ActivityType.HOME, home_edge, 0, int(departure_time)),
            Activity(
                ActivityType.SCHOOL, school_edge, int(departure_time), int(school_duration), mode
            ),
            Activity(
                ActivityType.HOME, home_edge, int(return_time), int(24 * 3600 - return_time), mode
            ),
        ]

    def save_edge_mapping(self, output_file: str):
        """Save OSM ID to edge mapping for future use"""
        print(f"Saving {len(self.osm_to_edge_cache)} edge mappings to {output_file}")
        with open(output_file, "w") as f:
            json.dump(self.osm_to_edge_cache, f, indent=2)
        print("Edge mapping cache saved successfully")
