...
class Activity:
    ...
    def __init__(self, activity_type: ActivityType, location_edge: str, 
                 start_time: int, duration: int, transport_mode: TransportMode = TransportMode.CAR):
...
class Agent:
    ...
    def update(self, sim_time: int):
        ...
        if (current_activity.completed and 
            sim_time >= current_activity.end_time and
            not self.in_transit):
    ...
    def _start_activity(self, activity: Activity, sim_time: int):
        ...
        if self.current_location != activity.location:
            self._initiate_trip(self.current_location, activity.location, sim_time, activity.transport_mode)
        else:
            activity.completed = True
    
    def _initiate_trip(self, from_edge: str, to_edge: str, depart_time: int, mode: TransportMode = TransportMode.CAR):
        ...
        try:
            ...
            if self.diagnostics:
                ...
                if self.diagnostics.failed_insertions <= 10:
                    print(f"Failed to insert {self.id} on edge {from_edge} with mode {mode.value}: {e}")
            self.in_transit = False
            self.vehicle_id = None
            current_activity = self.get_current_activity()
            if current_activity:
