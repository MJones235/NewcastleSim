...
class StationSimulationManager(SimulationManagerBase):
    ...
    def __init__(self, network_file: str, walking_areas_file: str, stops_file: str):
        ...
        self.spawn_interval = (
            2.0  # Increased to 2 seconds between spawns to avoid JuPedSim distance violations
        )
    ...
    def load_network(self):
        ...
        if self.network:
            ...
            self.pedestrian_edges = [
                edge for edge in self.network.getEdges() if edge.allows("pedestrian")
            ]
    ...
    def spawn_agents(self):
        ...
        print(
            f"Queued {len(self.agents_to_spawn)} agents for gradual spawning ({self.spawn_interval}s interval)"
        )
