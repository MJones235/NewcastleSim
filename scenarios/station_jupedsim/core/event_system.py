...
class SimulationEvent:
    ...
    def __eq__(self, other):
        if not isinstance(other, SimulationEvent):
            return False
        return self.time == other.time and self.action == other.action and self.value == other.value
...
class EventManager:
    ...
    def __init__(self, events_file: str | None = None) -> None:
        """
    ...
    def get_pending_events(self, sim_time: float) -> list[SimulationEvent]:
        ...
        """
        return [e for e in self.events if e not in self.triggered_events and e.time > sim_time]

    def get_next_event_time(self, sim_time: float) -> float | None:
        """
