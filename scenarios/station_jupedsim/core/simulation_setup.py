...
def setup_evacuation_exits(
    sim: StationSimulation, entrance_areas: dict[str, Polygon], exit_radius: float = 10.0
    ...
    for entrance_name, entrance_polygon in entrance_areas.items():
        try:
            ...
            print(
                f"  Created evacuation exit at '{entrance_name}' (exit_id={exit_id}, journey_id={journey_id})"
            )
...
def setup_platform_stages(
    sim: StationSimulation, platform_areas: dict[str, Polygon]
) -> tuple[dict[str, int], dict[str, int]]:
    """
    ...
    for platform_name, platform_polygon in platform_areas.items():
        ...
        try:
            ...
            stage_id = sim.stage_manager.create_waiting_stage(name=platform_name, position=position)
            ...
            print(
                f"  Created waiting stage for platform '{platform_name}' (stage_id={stage_id}, journey_id={journey_id})"
            )
        except Exception as e:
